...
<head>
    ...
    <title>{{ __('Prihlásenie - ChromaAi') }}</title>
...
<body class="antialiased text-slate-200">
    <div class="min-h-screen flex flex-col justify-center items-center px-6 py-12">
        <div class="mb-8 flex flex-col items-center">
            ...
            <h1 class="text-3xl font-bold tracking-tight text-white">{{ __('Vitajte späť') }}</h1>
        </div>
        ...
        <div class="w-full max-w-md bg-glass p-8 rounded-3xl shadow-2xl">
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autofocus class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all">
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center">
                        <input type="checkbox" name="remember" class="rounded border-slate-700 bg-slate-900 text-purple-600 focus:ring-purple-500">
                        <span class="ml-2 text-sm text-slate-400">{{ __('Zapamätať si ma') }}</span>
                    </label>
                </div>
            ...
            <div class="mt-8 text-center text-sm text-slate-500">
                {{ __('Nemáte ešte účet?') }}
                <a href="{{ route('register') }}" class="text-purple-400 hover:text-purple-300 font-semibold ml-1 transition-colors">{{ __('Zaregistrujte sa') }}</a>
            </div>
        </div>
    </div>
</body>
